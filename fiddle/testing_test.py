...
class DescribeDagDiffsTest(parameterized.TestCase):
  ...
  def test_no_diffs(self):
    x = make_test_value()
    y = make_test_value()
    self.assertEqual(testing.describe_dag_diffs(x, y), [])

  def test_type_diff(self):
    self.assertEqual(
        testing.describe_dag_diffs([0], ['0']),
  ...
  def test_set_diff(self):
    ...
    self.assertEqual(
        testing.describe_dag_diffs(x, y), ['* x.a={1, 2} but y.a={2, 3}'])

  def test_list_len_diff(self):
    self.assertEqual(
        testing.describe_dag_diffs([1], [2, 3]),
  ...
  def test_dict_keys_diff(self):
    self.assertEqual(
        testing.describe_dag_diffs(dict(a=1), dict(b=1)), [
            "* x['a'] has a value but y['a'] does not.",
            "* y['b'] has a value but x['b'] does not."
        ])
    self.assertEqual(
        testing.describe_dag_diffs([dict(a=2)], [dict(b=2)]), [
  ...
  def test_config_callable_diff(self):
    ...
    x = fdl.Config(TestClass)
    y = fdl.Config(test_func)
    self.assertEqual(
        testing.describe_dag_diffs(x, y), [
            '* x=<Config[TestClass()]> but y=<Config[test_func()]>',
        ])
  ...
  def test_sharing_not_detected_in_non_traversable_object(self):
    # There is no NodeTraverser registered for TestClass, so we don't
    # recurse inside it when looking for shared objects.
    shared_list = [1]
    x = TestClass(a=shared_list, b=shared_list)
    y = TestClass(a=shared_list, b=copy.copy(shared_list))
    self.assertEmpty(testing.describe_dag_diffs(x, y))
