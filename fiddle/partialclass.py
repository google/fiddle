...
def _make_meta_call_wrapper(cls: Type[object]):
  """Creates a `type(cls).__call__` wrapper that creates pickleable instances.
...
def partialclass(cls, *args, **kwargs):
  ...
  This class should generally behave interchangeably with `cls` in most settings
  (including with respect to `__name__`, `__doc__`, `__module__`, etc, which are
  all copied from `cls`). The method used here is to create a dynamic subclass
  of `cls`, with a metaclass which is itself a dynamic subclass of `cls`'s
  metaclass. This metaclass supplies partial parameters to `cls` during instance
  creation, with that result that constructing a `partial_cls` yields actual
  instances of `cls`.
